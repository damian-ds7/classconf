...
def _create_default_config(
    ...
    for config_class in config_classes:
        config_class_data = _get_class_fields(config_class)
...
def _get_class_fields(
    ...
    for field in fields(config_class):
        ...
        field_type = _unwrap_optional(resolved_types[field.name])
        value = _get_field_default_value(field, field_type)

        value = _serialize_field_value(value, field.name, field_serializers)
...
def _get_field_default_value(field: Any, field_type: Any) -> Any:
    ...
    if is_dataclass(field_type):
        return _get_class_fields(cast(type[ConfigClass], field_type))
...
def _get_root_configs(
    configs: list[type[ConfigClass[Any]]],
) -> list[type[ConfigClass[Any]]]:
    ...
    return [c for c in configs if c not in nested_configs]


def _convert_field_value(value: Any, field_type: type, parser: "ConfigParser") -> Any:
    if value is None:
        return None
    if is_dataclass(field_type):
        return _parse_config(cast(type[ConfigClass], field_type), value, parser)
...
def _parse_config(
    config_class: type[ConfigClass[T]],
    ...
    for field in fields(config_class):
        ...
        if key not in section_data:
            raise KeyError(f"Missing config key '{key}' for '{config_class.__name__}'")

        value = section_data[key]

        if field.name in field_deserializers:
            value = apply_deserializer(field_deserializers[field.name], value, parser)
        else:
            field_type = _unwrap_optional(resolved_types[field.name])
            value = _convert_field_value(value, field_type, parser)
...
class ConfigParser:
    ...
    def __init__(
        ...
        *configs: type[Any],
        ...
        self._configs = _validate_configs(*configs)
        self._config = self._read_config()

    def _read_config(self) -> dict[str, Any]:
        config_data = self._format.read(self._config_path)
        if config_data is None:
            config_data = _create_default_config(_get_root_configs(self._configs))
    ...
    def generate_config(
        ...
        *configs: Any,
        ...
        validated_types = _validate_configs(*types.keys())
        config = _create_default_config(validated_types)
    ...
    def add(self, *configs: type[Any]) -> None:
        self._configs = _validate_configs(*configs, *self._configs)
    ...
    def get(self, config_class: type[T]) -> T:
        ...
        if cast_class.__config__.top_level:
            ...
            return _parse_config(cast_class, config_data, self)
